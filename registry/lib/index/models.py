...
class SQLAlchemyIndex (Index):
    ...
    def _handle_repository_created(
        ...
        logger.debug('Index receive repository created message.')
...
class SQLAlchemyService (Index):
    ...
    def _handle_repository_created(self, sender, namespace, repository, value): #TODO Do not receive repository created message.
        logger.debug('Servcie receive service created message.')
        #name = '{0}/{1}'.format(namespace, repository)
        service = Service(name=repository, description='', icon='', type='')
        db.session.add(service)
        db.commit()

    def _handle_repository_updated(self, sender, namespace, repository, value): #TODO
        #name = '{0}/{1}'.format(namespace, repository)
        #description = ''
    ...
    def _handle_repository_deleted(self, sender, namespace, repository): #TODO
        name = '{0}/{1}'.format(namespace, repository)
    ...
    def add_service(self, data):
        ...
        timestamp = data.get('publish_time', time.time())
        ...
        service = Service(name = data['name'],
                          ...
                          compose_conf = data.get('compose_conf', ''),
...
class SQLAlchemyRecommend():
    ...
    def _setup_table(self):
        ...
        if len(results) == 0:
            service = None
            service = Service.query.filter(Service.name=='mysql').first()
            if service:
                db.session.add(Recommend(service=service, service_id=service.id))
            service = None
            service = Service.query.filter(Service.name=='nginx').first()
            if service:
                db.session.add(Recommend(service=service, service_id=service.id))
            db.session.commit()
        else:
...
class SQLAlchemyBanner():
    """
    Maintain an index of banner services data.
    """
    ...
    def _setup_table(self):
        ...
        logger.debug('setup banner table')
        results = Banner.query.all()
        if len(results) == 0:
            service = None
            service = Service.query.filter(Service.name == 'mysql').first()
            if service:
                db.session.add(Banner(service=service, service_id=service.id, banner_img_url=''))
            service = None
            service = Service.query.filter(Service.name == 'nginx').first()
            if service:
                db.session.add(Banner(service=service, service_id=service.id, banner_img_url=''))
    ...
    def results(self):
        services = Service.query.filter(Banner.service_id==Service.id)
        results = []
        if services is None:
            logger.debug('recommend table is empty')
        else:
            for service in services:
                banner = Banner.query.filter_by(service_id=service.id).first() #TODO Optimization
                timestamp = 0
                if service.publish_time != None:
                    timestamp = int(time.mktime(service.publish_time.utctimetuple()))
                obj = {
